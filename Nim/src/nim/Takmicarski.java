...
public class Takmicarski extends Strategija{

	public Takmicarski(int[] diskovi,int v,int prevmove,int maxdubina) {
	...
	private int minimax(int[] diskovi, int dubina,int prevmove, boolean maxigrac){
		if(isTerminal(diskovi)==1){
			if(maxigrac)return -32;
			else return 32;
		}
		if(dubina==maxdubina){
